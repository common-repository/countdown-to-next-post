/*******************************************************************************\
Countdown to Next Post JavaScript Module
Version 1.0
...
{
    var r = 0;
    for (var i=0; i < scott_timer_js_events.length; i++) {
		
		var nowDate = new Date();
		var targetDate = new Date(scott_timer_js_events[i]["targetDate"]*1000);
		//alert(document.getElementById(scott_timer_js_events[i]["id"]).innerHTML);
		if((targetDate - nowDate) < 0){
			document.getElementById(scott_timer_js_events[i]["id"]).innerHTML = sprintf(scott_timer_js_language['ago'], scott_timer_fuzzyDate(nowDate, targetDate, getOptions));			
		}
		else if((targetDate - nowDate) >= 0 ){
			document.getElementById(scott_timer_js_events[i]["id"]).innerHTML = sprintf(scott_timer_js_language['in'],scott_timer_fuzzyDate(targetDate, nowDate, getOptions));
		}
	}
	
    window.setTimeout('scott_timer_js()', 1000);
}

function scott_timer_fuzzyDate(targetTime, nowTime, getOptions){
	var rollover = 0;
	var vars = '';
	var sigNumHit = false;
	var totalTime = 0;
	...
	//Year
	if(getOptions['displayYear']){
		if(sigNumHit || !getOptions['stripZero'] || resultantYear){
			if(resultantYear==1){
				s = sprintf(scott_timer_js_language['year'], resultantYear) + ' ';
			} else{
				s = sprintf(scott_timer_js_language['years'], resultantYear) + ' ';
			}
			sigNumHit = true;
		}
	}
	...
	if(getOptions['displayMonth']){
		if(sigNumHit || !getOptions['stripZero'] || (resultantMonth + parseInt(rollover/2628000)) ){
			...
			if(resultantMonth==1){
				s = s + sprintf(scott_timer_js_language['month'], resultantMonth) + ' ';
			} else{
				s = s + sprintf(scott_timer_js_language['months'], resultantMonth) + ' ';
			}
	...
	if(getOptions['displayWeek']){
		if(sigNumHit || !getOptions['stripZero'] || parseInt( (resultantDay + parseInt(rollover/86400) )/7)){
			...
			if((parseInt( (resultantDay + parseInt(rollover/86400) )/7))==1){
				s = s + sprintf(scott_timer_js_language['week'], (parseInt( (resultantDay + parseInt(rollover/86400) )/7))) + ' ';
			} else{
				s = s + sprintf(scott_timer_js_language['weeks'], (parseInt( (resultantDay + parseInt(rollover/86400) )/7))) + ' ';
			}
	...
	//Day
	if(getOptions['displayDay']){
		if(sigNumHit || !getOptions['stripZero'] || (resultantDay + parseInt(rollover/86400)) ){
			resultantDay = resultantDay + parseInt(rollover/86400);
			if(resultantDay==1){
				s = s + sprintf(scott_timer_js_language['day'], resultantDay) + ' ';
			} else{
				s = s + sprintf(scott_timer_js_language['days'], resultantDay) + ' ';
			}
			rollover = rollover - parseInt(rollover/86400)*86400;
			sigNumHit = true;
		}
	...
	//Hour
	if(getOptions['displayHour']){
		if(sigNumHit || !getOptions['stripZero'] || (resultantHour + parseInt(rollover/3600)) ){
			resultantHour = resultantHour + parseInt(rollover/3600);
			if(resultantHour==1){
				s = s + sprintf(scott_timer_js_language['hour'], resultantHour) + ' ';
			} else{
				s = s + sprintf(scott_timer_js_language['hours'], resultantHour) + ' ';
			}
	...
	if(getOptions['displayMinute']){
		if(sigNumHit || !getOptions['stripZero'] || (resultantMinute + parseInt(rollover/60)) ){
			...
			if(resultantMinute==1){
				s = s + sprintf(scott_timer_js_language['minute'], resultantMinute) + ' ';
			} else{
				s = s + sprintf(scott_timer_js_language['minutes'], resultantMinute) + ' ';
			}
	...
	//Second
	if(getOptions['displaySecond']){
		if(resultantSecond==1){
			s = s + sprintf(scott_timer_js_language['second'], resultantSecond) + ' ';
		} else{
			s = s + sprintf(scott_timer_js_language['seconds'], resultantSecond) + ' ';
		}
	}
	...
	if(s==''){
		if(getOptions['displaySecond']){
			s = sprintf(scott_timer_js_language['seconds'], 0);
		}
		else if(getOptions['displayMinute']){
			s = sprintf(scott_timer_js_language['minutes'], 0);
		}
		else if(getOptions['displayHour']){
			s = sprintf(scott_timer_js_language['hours'], 0);
		}	
		else if(getOptions['displayDay']){
			s = sprintf(scott_timer_js_language['days'], 0);
		}
		else if(getOptions['displayWeek']){
			s = sprintf(scott_timer_js_language['weeks'], 0);
		}
		else if(getOptions['displayMonth']){
			s = sprintf(scott_timer_js_language['months'], 0);
		}
		else{
			s = sprintf(scott_timer_js_language['years'], 0);
		}
	}
